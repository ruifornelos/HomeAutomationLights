...
class ControlActivity : AppCompatActivity(), SensorEventListener {
    ...
    override fun onCreate(savedInstanceState: Bundle?) {
        ...
        switchPower?.setOnCheckedChangeListener { _, onSwitch ->
            if (onSwitch) {
                if(roomSelected == 0){
                    Toast.makeText(this, getString(R.string.noRoomSelected), Toast.LENGTH_SHORT).show()
                } else {
                    ledStateChange(roomSelected)
                    Toast.makeText(this, getString(R.string.ledsON), Toast.LENGTH_SHORT).show()
                }
            } else {
                if(roomSelected == 0){
                    Toast.makeText(this, getString(R.string.noRoomSelected), Toast.LENGTH_SHORT).show()
        ...
        switchMode?.setOnCheckedChangeListener { _, onSwitch ->
            if (onSwitch) {
                if(roomSelected == 0){
                    ...
                    resume = true
                    Toast.makeText(this, getString(R.string.automatic_mode), Toast.LENGTH_SHORT).show()
                }
            } else {
                resume = false
        ...
        when(room){
            1 -> {
                bathroomState = if(!bathroomState){
                    roomSelected = 1
                    ConnectionManager.writeCharacteristic(device,bedroomChar,"bathroom 255".toByteArray(Charsets.UTF_8))
                    true
                } else {
                    roomSelected = 0
                    ConnectionManager.writeCharacteristic(device,bedroomChar,"bathroom 0".toByteArray(Charsets.UTF_8))
                    false
                }
            }
            2 -> {
                kitchenState = if(!kitchenState){
                    ConnectionManager.writeCharacteristic(device,bedroomChar,"kitchen 255".toByteArray(Charsets.UTF_8))
                    true
                } else {
                    ConnectionManager.writeCharacteristic(device,bedroomChar,"kitchen 0".toByteArray(Charsets.UTF_8))
                    false
                }
            }
            3 -> {
                livingRoomState = if(!livingRoomState){
                    ConnectionManager.writeCharacteristic(device,bedroomChar,"living 255".toByteArray(Charsets.UTF_8))
                    true
                } else {
                    ConnectionManager.writeCharacteristic(device,bedroomChar,"living 0".toByteArray(Charsets.UTF_8))
                    false
                }
            }
            4 ->{
                bedroomState = if(!bedroomState){
                    ConnectionManager.writeCharacteristic(device,bedroomChar,"bedroom 255".toByteArray(Charsets.UTF_8))
                    true
                } else {
                    ConnectionManager.writeCharacteristic(device,bedroomChar,"bedroom 0".toByteArray(Charsets.UTF_8))
                    false
                }
            }
